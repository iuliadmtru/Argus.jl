...
Stack used for backtracking during matching.
"""
struct RecoveryStack{T}
    st::Vector{T}
end
RecoveryStack{T}() where T = RecoveryStack(T[])
_RecoveryStack() = RecoveryStack{Tuple{SyntaxPatternNode, JS.SyntaxNode, BindingSet}}()
_RecoveryStack_vec() =
    RecoveryStack{Tuple{Vector{SyntaxPatternNode}, Vector{JS.SyntaxNode}, BindingSet}}()

# Base overwrites

Base.push!(st::RecoveryStack{T}, el::T) where T = push!(st.st, el)
Base.pop!(st::RecoveryStack{T}) where T = pop!(st.st)
Base.popfirst!(st::RecoveryStack{T}) where T = popfirst!(st.st)
Base.append!(st1::RecoveryStack{T}, st2::RecoveryStack{T}) where T =
    append!(st1.st, st2.st)
Base.length(st::RecoveryStack) = length(st.st)
Base.isempty(st::RecoveryStack) = isempty(st.st)
Base.iterate(st::RecoveryStack) = iterate(st.st)
Base.iterate(st::RecoveryStack, i::Int) = iterate(st.st, i)
...
function _syntax_match(pattern::SyntaxPatternNode,
    ...
    recovery_stack_cs = _RecoveryStack_vec()
...
function partial_syntax_match(pattern_nodes::Vector{SyntaxPatternNode},
    ...
    if isempty(srcs)
        ...
        return recover!(recovery_stack,                     # `recovery_stack` empty => 1.b
                        ...
                        fail_ret=(MatchFail(), srcs),  # TODO: Add fail location and file??
                        greedy,
                        tmp,
                        rep_sequence)
    end
    ...
    s = srcs[1]
    if !is_rep(p)
        recovery_stack_current_p = _RecoveryStack()
        ...
        if !rep_sequence
            ...
            recovery_stack_rest = _RecoveryStack_vec()
            ...
            if isempty(recovery_stack_current_p)
                recovery_stack_rest =
                    resolve_conflicts_and_combine(RecoveryStack([([p], [s], bindings)]),
                                                  recovery_stack_rest)
            else
...
function syntax_match_or(or_node::SyntaxPatternNode,
    ...
    for (i, p) in enumerate(children(or_node))
        ...
        recovery_stack_branch = _RecoveryStack()
        match_result = _syntax_match(p,
                                     src,
                                     bindings_alt;
...
function syntax_match_and(and_node::SyntaxPatternNode,
    ...
    for (i, p) in enumerate(children(and_node))
        ...
        branch_recovery_stack = _RecoveryStack()
        match_result = _syntax_match(p,
                                     src,
                                     bindings;
...
"""
    syntax_match_inside(inside_node::SyntaxPatternNode,
                        src::JS.SyntaxNode,
                        bindings::BindingSet;
                        recovery_stack=[],
                        greedy=true,
...
"""
    syntax_match_contains(contains_node::SyntaxPatternNode,
                          src::JS.SyntaxNode,
                          bindings::BindingSet;
                          recovery_stack=[],
                          greedy=true,
...
"""
function recover!(recovery_stack::RecoveryStack,
                  from::Function;
                  recover=true,
                  popfirst=false,
                  fail_ret,
...
"""
function resolve_conflicts_and_combine(st1::RecoveryStack, st2::RecoveryStack)
    final_st = []
