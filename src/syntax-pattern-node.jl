...
is_inside(ex)   = is_pattern_form(ex) && ex.args[2].args[1] === :inside
is_contains(ex) = is_pattern_form(ex) && ex.args[2].args[1] === :contains
...
function _get_pattern_form_arg_nodes(node::JS.SyntaxNode)
    ...
    name === :not      && return _strip_node.(args)
    name === :inside   && return _strip_node.(args)
    name === :contains && return _strip_node.(args)
...
is_inside(node::SyntaxPatternNode)   = isa(node.data, InsideSyntaxData)
is_contains(node::SyntaxPatternNode) = isa(node.data, ContainsSyntaxData)

is_cond(node::SyntaxPatternNode) = is_fail(node) || is_when(node)
...
function _get_fail_args(args::T) where T <: AbstractVector{JS.SyntaxNode}
    length(args) == 3 ||
        throw(SyntaxError("""
                          invalid `~fail` syntax
    ...
    fail_message = _strip_node(args[3]).data.val
    isa(fail_message, String) ||
        throw(SyntaxError("""
                          invalid `~fail` syntax
                          The last argument should be a `String`.""",
                          JS.filename(args[3]),
                          JS.source_location(args[3])[1],
                          JS.source_location(args[3])[2] - 1))
    return [_get_symbols(args[1], "~fail"),
            JS.to_expr(args[2]),
            fail_message]
end
...
function _get_symbols(node::JS.SyntaxNode, pattern_form_name::String)
    error = SyntaxError("""
                        ...
                        """,
                        JS.filename(node),
                        JS.source_location(node)[1],
                        JS.source_location(node)[2] - 1)
